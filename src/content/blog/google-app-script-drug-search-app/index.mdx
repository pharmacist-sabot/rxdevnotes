...
<FeatureGrid>
  <FeatureCard title="Google Sheets คือฐานข้อมูล" icon="circle-stack">
    ...
    ไม่ว่าจะเป็นการเพิ่มรายการยาใหม่หรือลบรายการยาเดิม หากเราทำการเปลี่ยนแปลงที่
    Google Sheets แล้ว ระบบจะอัปเดตข้อมูลในเว็บแอปโดยอัตโนมัติ
  </FeatureCard>
  <FeatureCard title="Apps Script คือ Backend" icon="cpu-chip">
    Google Apps Script จะทำหน้าที่เป็นโค้ดฝั่ง "หลังบ้าน" หรือที่เราเรียกว่า
    Backend ทั้งหมด คอยรับคำสั่งค้นหาจากหน้าเว็บ
    แล้วไปดึงข้อมูลจากชีตส่งกลับไปให้ผู้ใช้งานสามารถดูข้อมูลที่อยู่ใน google
    sheet ได้
  </FeatureCard>
  ...
  try {
    ...
    const results = data
      .filter((row) => {
        ...
        const matchNed = ned ? row[4] === ned : true;
